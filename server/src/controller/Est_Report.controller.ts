...
export async function handleScoreExerciseSummary(req : Request, res : Response) {
    try {   
        ...
        const result = await ScoreExerciseSummary({user_id, workout_plan_code, start_date, end_date, page, limit, sortDir });
        res.json(result);
    } catch (error : any) {
...
export async function handleExerciseMusclePlanList(req : Request, res : Response) {
    try {
        ...
        const limit = Number(req.query.limit) || 10;
        const sortDir = String(req.query.sortDir || "asc");
        const result = await ExerciseMusclePlanList({user_id, muscle_area, page, limit, sortDir});
        res.json(result);
    } catch (error : any) {
        res.status(500).json({message:error.message});
    }
...
export async function handleWorkOutDistribution(req : Request, res : Response) {
    try {
        ...
        const limit = Number(req.query.limit) || 10;
        const sortDir = String(req.query.sortDir || "asc");
        const result = await WorkOutDistribution({user_id, page, limit, sortDir});
        res.json(result);
    } catch (error : any) {
        res.status(500).json({message:error.message});
    }
